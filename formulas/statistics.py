...
def process_continuous_plot_data(data: ContinuousData):
    ...
    f = {'lines': [], 'dotsx': [], 'dotsy': []}
    func = '0,\tпри x <= ' + str(round(data.intervals[0][0], 3)) + '\n'

    # Начальный отрезок линии перед первым интервалом
    intlen = 3 * (data.intervals[0][1] - data.intervals[0][0])
    line = [[data.intervals[0][0] - intlen, data.intervals[0][0]], [0, 0]]
    f['lines'].append(line)

    # Инициализация счетчика первым весом
    counter = data.W[0]

    for i in range(k-1):
        # Создание строки кусочной функции для текущего интервала
        newstr = (
            str(round(counter, 2)) + ',\tпри ' +
            str(round(data.intervals[i][0], 3)) + ' < x <= ' +
            str(round(data.intervals[i][1], 3))
        )
        
        # Определение отрезка линии для текущего интервала
        line = [
            [data.intervals[i][0], data.intervals[i][1]],
            [counter, counter]
        ]
        f['lines'].append(line)

        # Добавление координат точек
        f['dotsx'].append(data.intervals[i][0])
        f['dotsy'].append(counter)

        # Обновление счетчика добавлением текущего веса
        counter += data.W[i+1]
        func += newstr + '\n'
    
    # Добавление заключительной части кусочной функции
    func += '1,\tпри x > ' + str(round(data.intervals[k - 1][0], 3))

    # Добавление финального отрезка линии за последним интервалом
    line = [
        [data.intervals[k-1][0], data.intervals[k-1][0] + intlen],
        [counter, counter]
    ]
    f['lines'].append(line)

    # Добавление последних координат точек
    f['dotsx'].append(data.intervals[k-1][0])
    f['dotsy'].append(counter)

    # Сохранение кусочной функции и данных для графика в словарь результатов
    plot_data['F*'] = func
    plot_data['func'] = f
